...
class GBRModelTrainer:
    def __init__(self, data_path: str = None, output_base_dir: str = "outputs", seed: int = 42):
        ...
        self.output_dir = os.path.join(output_base_dir, f"gbr_gjo_{self.timestamp}")
        ...
        logger.info(f"GBRModelTrainer initialized")
    ...
    def optimize_hyperparameters(self, epochs: int = 5, pop_size: int = 5):
        ...
        # Declare search space for Gradient Boosting Regressor
        bounds = [
            ...
            IntegerVar(lb=3, ub=10, name="min_samples_leaf"),
            FloatVar(lb=0.1, ub=1.0, name="subsample"),
            FloatVar(lb=0.1, ub=1.0, name="max_features"),
            FloatVar(lb=0.0001, ub=0.99, name="alpha")
        ]

        # Fitness function (5-fold CV RMSE on train data)
        def cv_rmse(solution):
            (max_depth, learning_rate, n_estimators, min_samples_split,
             min_samples_leaf, subsample, max_features, alpha) = solution
        ...
        self.best_params = {
            ...
            "min_samples_split": int(self.g_best.solution[3]),
            "min_samples_leaf": int(self.g_best.solution[4]),
            "subsample": self.g_best.solution[5],
            "max_features": self.g_best.solution[6],
            "alpha": self.g_best.solution[7],
            "random_state": self.seed,
            "loss": 'squared_error'
        }
    ...
    def train_final_model(self):
        """Train final GBR model with optimized hyperparameters"""
        ...
        model_path = os.path.join(self.output_dir, f"gbr_model_{self.timestamp}.pkl")
    ...
    def _save_predictions(self, y_true, y_pred, dataset_type: str):
        ...
        predictions_df = pd.DataFrame({
            "Actual": y_true,
            "GBR_Predicted": y_pred
        })

        filename = os.path.join(self.output_dir, f"gbr_{dataset_type}_predictions_{self.timestamp}.csv")
    ...
    def _save_metrics(self, train_metrics: Dict, test_metrics: Dict):
        ...
        filename = os.path.join(self.output_dir, f"gbr_metrics_{self.timestamp}.txt")

        with open(filename, 'w') as file:
            file.write("=" * 50 + "\n")
            file.write(f"Gradient Boosting Model Training Results\n")
    ...
    def plot_feature_importance(self):
        ...
        plt.title('Ranking of Features (Gradient Boosting)', fontsize=16)
    ...
    def plot_scatter_with_fit(self, y_true, y_pred, dataset_type: str):
        ...
        # Labels
        plt.title(f'{dataset_type} Set: Predicted vs Actual (Gradient Boosting)', fontsize=16)
    ...
    def run_full_pipeline(self, target_column: str = "CCR_ES", epochs: int = 5, pop_size: int = 5):
        ...
        logger.info("Starting Gradient Boosting full pipeline...")
        ...
        logger.info("✅ Gradient Boosting pipeline completed!")
...
def run_gbr_pipeline(data_path: str = None, output_base_dir: str = "outputs",
                     target_column: str = "CCR_ES", seed: int = 42,
                     epochs: int = 5, pop_size: int = 5):

    trainer = GBRModelTrainer(data_path, output_base_dir, seed)

...
class AdaBoostModelTrainer:
    def __init__(self, data_path: str = None, output_base_dir: str = "outputs", seed: int = 42):
        ...
        self.output_dir = os.path.join(output_base_dir, f"ada_gjo_{self.timestamp}")
        ...
        logger.info(f"AdaBoostModelTrainer initialized")
    ...
    def optimize_hyperparameters(self, epochs: int = 5, pop_size: int = 5):
        ...
        # Declare search space for AdaBoost Regressor
        bounds = [
            IntegerVar(lb=50, ub=1000, name="n_estimators"),
            FloatVar(lb=0.1, ub=2.0, name="learning_rate"),
            FloatVar(lb=0.1, ub=2.0, name="loss"),
            IntegerVar(lb=3, ub=10, name="max_depth")
        ]

        # Fitness function (5-fold CV RMSE on train data)
        def cv_rmse(solution):
            (n_estimators, learning_rate, loss, max_depth) = solution

            # Map loss float to actual loss strings
        ...
        # Map the optimized loss parameter to string
        loss_value = float(self.g_best.solution[2])
        if loss_value < 0.7:
            loss_str = 'linear'
        elif loss_value < 1.4:
            loss_str = 'square'
        ...
        self.best_params = {
            ...
            "learning_rate": self.g_best.solution[1],
            "loss": loss_str,
            "random_state": self.seed,
            "estimator": DecisionTreeRegressor(
                max_depth=int(self.g_best.solution[3]),
                random_state=self.seed
            )
        }
        ...
        for key, value in self.best_params.items():
            if key == "estimator":
                logger.info(f"  estimator: DecisionTreeRegressor(max_depth={value.max_depth})")
            else:
    ...
    def train_final_model(self):
        """Train final AdaBoost model with optimized hyperparameters"""
        logger.info("Training final AdaBoost model...")

        self.model = AdaBoostRegressor(**self.best_params)
        self.model.fit(self.X_train, self.y_train)

        # Save the trained model
        model_path = os.path.join(self.output_dir, f"ada_model_{self.timestamp}.pkl")
    ...
    def _save_predictions(self, y_true, y_pred, dataset_type: str):
        ...
        predictions_df = pd.DataFrame({
            "Actual": y_true,
            "AdaBoost_Predicted": y_pred
        })

        filename = os.path.join(self.output_dir, f"ada_{dataset_type}_predictions_{self.timestamp}.csv")
    ...
    def _save_metrics(self, train_metrics: Dict, test_metrics: Dict):
        ...
        filename = os.path.join(self.output_dir, f"ada_metrics_{self.timestamp}.txt")

        with open(filename, 'w') as file:
            file.write("=" * 50 + "\n")
            file.write(f"AdaBoost Model Training Results\n")
            ...
            for key, value in self.best_params.items():
                if key == "estimator":
                    file.write(f"  estimator: DecisionTreeRegressor(max_depth={value.max_depth})\n")
                else:
    ...
    def plot_feature_importance(self):
        ...
        plt.title('Ranking of Features (AdaBoost)', fontsize=16)
    ...
    def plot_scatter_with_fit(self, y_true, y_pred, dataset_type: str):
        ...
        # Labels
        plt.title(f'{dataset_type} Set: Predicted vs Actual (AdaBoost)', fontsize=16)
    ...
    def perform_shap_analysis(self):
        ...
        try:
            # Create KernelExplainer for AdaBoost
            ...
            X_test_sample = self.X_test[:n_samples]
            ...
            logger.warning(f"SHAP analysis failed for AdaBoost: {e}")
            logger.warning("SHAP analysis may be computationally expensive for AdaBoost")
            return None, None

    def run_full_pipeline(self, target_column: str = "CCR_ES", epochs: int = 5, pop_size: int = 5):

        logger.info("Starting AdaBoost full pipeline...")
        ...
        logger.info("✅ AdaBoost pipeline completed!")
...
def run_ada_pipeline(data_path: str = None, output_base_dir: str = "outputs",
                     target_column: str = "CCR_ES", seed: int = 42,
                     epochs: int = 5, pop_size: int = 5):

    trainer = AdaBoostModelTrainer(data_path, output_base_dir, seed)

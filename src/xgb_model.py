...
class XGBModelTrainer:
    def __init__(self, data_path: str = None, output_base_dir: str = "outputs", seed: int = 42):
        ...
        self.output_dir = os.path.join(output_base_dir, f"xgb_gjo_{self.timestamp}")
        ...
        logger.info(f"XGBModelTrainer initialized")
    ...
    def optimize_hyperparameters(self, epochs: int = 5, pop_size: int = 5):
        ...
        self.best_params = {
            ...
            "min_child_weight": int(self.g_best.solution[3]),
            "subsample": self.g_best.solution[4],
            "colsample_bytree": self.g_best.solution[5],
            "reg_lambda": self.g_best.solution[6],
    ...
    def train_final_model(self):
        ...
        model_path = os.path.join(self.output_dir, f"xgb_model_{self.timestamp}.pkl")
    ...
    def _save_predictions(self, y_true, y_pred, dataset_type: str):
        ...
        filename = os.path.join(self.output_dir, f"xgb_{dataset_type}_predictions_{self.timestamp}.csv")
    ...
    def _save_metrics(self, train_metrics: Dict, test_metrics: Dict):
        ...
        filename = os.path.join(self.output_dir, f"xgb_metrics_{self.timestamp}.txt")

        with open(filename, 'w') as file:
            file.write("=" * 50 + "\n")
            file.write(f"XGBoost Model Training Results\n")
    ...
    def plot_feature_importance(self):
        ...
        plt.title('Ranking of Features (XGBoost)', fontsize=16)
    ...
    def plot_scatter_with_fit(self, y_true, y_pred, dataset_type: str):
        ...
        # Labels
        plt.title(f'{dataset_type} Set: Predicted vs Actual (XGBoost)', fontsize=16)
    ...
    def run_full_pipeline(self, target_column: str = "CCR_ES", epochs: int = 5, pop_size: int = 5):
        ...
        logger.info("Starting XGBoost full pipeline...")
...
def run_xgb_pipeline(data_path: str = None, output_base_dir: str = "outputs",
                     target_column: str = "CCR_ES", seed: int = 42,
                     epochs: int = 5, pop_size: int = 5):

    trainer = XGBModelTrainer(data_path, output_base_dir, seed)

...
class RFModelTrainer:
    def __init__(self, data_path: str = None, output_base_dir: str = "outputs", seed: int = 42):
        ...
        self.output_dir = os.path.join(output_base_dir, f"rf_gjo_{self.timestamp}")
        ...
        logger.info(f"RFModelTrainer initialized")
    ...
    def optimize_hyperparameters(self, epochs: int = 5, pop_size: int = 5):
        ...
        # Declare search space for Random Forest Regressor
        bounds = [
            ...
            IntegerVar(lb=3, ub=10, name="max_leaf_nodes")
        ]

        # Fitness function (5-fold CV RMSE on train data)
        def cv_rmse(solution):
            (n_estimators, max_depth, min_samples_split, min_samples_leaf, max_features, max_leaf_nodes) = solution
            ...
            params = dict(
                ...
                max_leaf_nodes=None if int(max_leaf_nodes) == 10 else int(max_leaf_nodes),
        ...
        self.best_params = {
            ...
            "max_leaf_nodes": None if int(self.g_best.solution[5]) == 10 else int(self.g_best.solution[5]),
            "random_state": self.seed,
            "n_jobs": -1
        }
    ...
    def train_final_model(self):
        ...
        model_path = os.path.join(self.output_dir, f"rf_model_{self.timestamp}.pkl")
    ...
    def _save_predictions(self, y_true, y_pred, dataset_type: str):
        ...
        filename = os.path.join(self.output_dir, f"rf_{dataset_type}_predictions_{self.timestamp}.csv")
    ...
    def _save_metrics(self, train_metrics: Dict, test_metrics: Dict):
        ...
        filename = os.path.join(self.output_dir, f"rf_metrics_{self.timestamp}.txt")

        with open(filename, 'w') as file:
            file.write("=" * 50 + "\n")
            file.write(f"Random Forest Model Training Results\n")
    ...
    def plot_feature_importance(self):
        ...
        plt.title('Ranking of Features (Random Forest)', fontsize=16)
    ...
    def plot_scatter_with_fit(self, y_true, y_pred, dataset_type: str):
        ...
        # Labels
        plt.title(f'{dataset_type} Set: Predicted vs Actual (Random Forest)', fontsize=16)
    ...
    def run_full_pipeline(self, target_column: str = "CCR_ES", epochs: int = 5, pop_size: int = 5):
        ...
        logger.info("Starting Random Forest full pipeline...")
        ...
        logger.info("✅ Random Forest pipeline completed!")
...
def run_rf_pipeline(data_path: str = None, output_base_dir: str = "outputs",
                    target_column: str = "CCR_ES", seed: int = 42,
                    epochs: int = 5, pop_size: int = 5):

    trainer = RFModelTrainer(data_path, output_base_dir, seed)

...
class LGBMModelTrainer:
    def __init__(self, data_path: str = None, output_base_dir: str = "outputs", seed: int = 42):
        ...
        self.output_dir = os.path.join(output_base_dir, f"lgbm_gjo_{self.timestamp}")
        ...
        logger.info(f"LGBMModelTrainer initialized")
    ...
    def load_and_prepare_data(self, target_column: str = "CCR_ES"):
        ...
        # Features (X) - Keep as DataFrame for LightGBM column names
        self.X = self.data.drop(columns=existing_columns)
    ...
    def optimize_hyperparameters(self, epochs: int = 5, pop_size: int = 5):
        ...
        # Declare search space for LightGBM Regressor
        bounds = [
            IntegerVar(lb=4, ub=10, name="max_depth"),
            FloatVar(lb=0.01, ub=0.3, name="learning_rate"),
            IntegerVar(lb=100, ub=1500, name="n_estimators"),
            IntegerVar(lb=20, ub=200, name="num_leaves"),
            IntegerVar(lb=3, ub=10, name="min_child_samples"),
            FloatVar(lb=0.1, ub=1.0, name="subsample"),
            FloatVar(lb=0.1, ub=1.0, name="colsample_bytree"),
            FloatVar(lb=0.0001, ub=0.99, name="reg_alpha")
        ]

        # Fitness function (5-fold CV RMSE on train data)
        def cv_rmse(solution):
            (max_depth, learning_rate, n_estimators, num_leaves,
             min_child_samples, subsample, colsample_bytree, reg_alpha) = solution
            ...
            for tr_idx, val_idx in kf.split(self.X_train):
                ...
                fold_rmse.append(np.sqrt(mean_squared_error(self.y_train.iloc[val_idx], preds)))
        ...
        self.best_params = {
            "max_depth": int(self.g_best.solution[0]),
            "learning_rate": self.g_best.solution[1],
            "n_estimators": int(self.g_best.solution[2]),
            "num_leaves": int(self.g_best.solution[3]),
            "min_child_samples": int(self.g_best.solution[4]),
            "subsample": self.g_best.solution[5],
            "colsample_bytree": self.g_best.solution[6],
            "reg_alpha": self.g_best.solution[7],
            "random_state": self.seed,
            "objective": 'regression',
            "metric": 'rmse',
            "boosting_type": 'gbdt',
    ...
    def train_final_model(self):
        """Train final LightGBM model with optimized hyperparameters"""
        ...
        model_path = os.path.join(self.output_dir, f"lgbm_model_{self.timestamp}.pkl")
    ...
    def _save_predictions(self, y_true, y_pred, dataset_type: str):
        ...
        predictions_df = pd.DataFrame({
            "Actual": y_true,
            "LGBM_Predicted": y_pred
        })

        filename = os.path.join(self.output_dir, f"lgbm_{dataset_type}_predictions_{self.timestamp}.csv")
    ...
    def _save_metrics(self, train_metrics: Dict, test_metrics: Dict):
        ...
        filename = os.path.join(self.output_dir, f"lgbm_metrics_{self.timestamp}.txt")

        with open(filename, 'w') as file:
            file.write("=" * 50 + "\n")
            file.write(f"LightGBM Model Training Results\n")
    ...
    def plot_feature_importance(self):
        ...
        plt.title('Ranking of Features (LightGBM)', fontsize=16)
    ...
    def plot_scatter_with_fit(self, y_true, y_pred, dataset_type: str):
        ...
        # Labels
        plt.title(f'{dataset_type} Set: Predicted vs Actual (LightGBM)', fontsize=16)
    ...
    def run_full_pipeline(self, target_column: str = "CCR_ES", epochs: int = 5, pop_size: int = 5):
        ...
        logger.info("Starting LightGBM full pipeline...")
        ...
        logger.info("✅ LightGBM pipeline completed!")
...
def run_lgbm_pipeline(data_path: str = None, output_base_dir: str = "outputs",
                      target_column: str = "CCR_ES", seed: int = 42,
                      epochs: int = 5, pop_size: int = 5):

    trainer = LGBMModelTrainer(data_path, output_base_dir, seed)
